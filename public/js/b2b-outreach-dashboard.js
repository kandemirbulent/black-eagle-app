(function (root, factory) {
  ...
  function createController({ authFetch, showMessage, actorRole = "ADMIN", documentRef = document, windowRef = typeof window !== "undefined" ? window : { innerWidth: 1280, innerHeight: 800, addEventListener() {} }, sleep = wait, pollIntervalMs = 1500, timeoutMs = 90000, refreshAfterImport, operationOverride }) {
    ...
    const state = { page: 1, limit: 25, total: 0, selectedCount: 0, unavailableCount: 0, allResultsSelected: false, contacts: [], selected: new Map(), researchSelected: new Set(), editContact: null, activeResearchRequestId: "", researchPollTimer: null, draft: null, draftContact: null, importBatchId: "", importConfirming: false, importResize: null, tooltipControl: null, tooltipPinned: false };
    ...
    const editFields = [["decisionMakerName", "b2bEditDecisionMakerName"], ["companyName", "b2bEditCompanyName"], ["role", "b2bEditRole"], ["businessEmail", "b2bEditBusinessEmail"], ["phone", "b2bEditPhone"], ["officialWebsite", "b2bEditOfficialWebsite"], ["segment", "b2bEditSegment"], ["bestBlackEagleOffer", "b2bEditBestOffer"], ["sourceUrl", "b2bEditSourceUrl"], ["notes", "b2bEditNotes"]];
    function openContactEditor(contact) { if (!isSuperadmin) return; state.editContact = contact; for (const [field, elementId] of editFields) el(elementId).value = contact?.[field] || ""; el("b2bEditContactModal").classList.remove("hidden"); }
    function closeContactEditor() { state.editContact = null; el("b2bEditContactModal").classList.add("hidden"); }
    async function saveContact() { if (!isSuperadmin || !state.editContact) return null; const fields = {}; for (const [field, elementId] of editFields) fields[field] = el(elementId).value; const updated = await json(`/api/admin/b2b-outreach/contacts/${encodeURIComponent(idOf(state.editContact))}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(fields) }); closeContactEditor(); await loadContacts(); status("Contact updated."); return updated; }
    ...
    function filters() {
      ...
      const sorts = sortValues(); if (sorts.length) payload.sort = sorts.join(",");
    ...
    const sortField = (value) => ({ UNSENT_FIRST: "SENT_STATUS", SENT_FIRST: "SENT_STATUS", REPLIED_FIRST: "REPLY_STATUS", AWAITING_REPLY_FIRST: "REPLY_STATUS" })[value] || String(value || "").replace(/_(ASC|DESC)$/, "");
    const sortControls = () => [el("b2bSort"), ...Array.from(el("b2bSortExtras")?.querySelectorAll?.("select") || [])].filter(Boolean);
    const sortValues = () => sortControls().map((select) => select.value).filter(Boolean);
    function refreshSortControls() { const controls = sortControls(), selected = controls.map((select) => sortField(select.value)).filter(Boolean); for (const select of controls) for (const option of Array.from(select.options || [])) option.disabled = Boolean(option.value) && sortField(option.value) !== sortField(select.value) && selected.includes(sortField(option.value)); if (el("b2bAddSort")) el("b2bAddSort").disabled = controls.length >= 3; }
    function sortChanged(select) { if (select === el("b2bSort") && !select.value) el("b2bSortExtras")?.replaceChildren(); else if (select !== el("b2bSort") && select.value && !el("b2bSort")?.value) { el("b2bSort").value = select.value; select.closest?.("label")?.remove(); } refreshSortControls(); state.page = 1; loadContacts().catch(handleError); }
    function addSortControl() { if (sortControls().length >= 3) return; const label = documentRef.createElement("label"), select = documentRef.createElement("select"), remove = documentRef.createElement("button"); label.textContent = `Sort ${sortControls().length + 1}`; select.className = "b2b-sort-extra"; select.innerHTML = el("b2bSort").innerHTML; select.remove?.(0); remove.type = "button"; remove.className = "btn btn-secondary"; remove.textContent = "×"; remove.setAttribute("aria-label", "Remove sort"); select.addEventListener("change", () => sortChanged(select)); remove.addEventListener("click", () => { label.remove?.(); refreshSortControls(); state.page = 1; loadContacts().catch(handleError); }); label.append(select, remove); el("b2bSortExtras").appendChild(label); refreshSortControls(); state.page = 1; loadContacts().catch(handleError); }
    ...
    function renderContacts() {
      ...
      for (const contact of state.contacts) {
        ...
        const action = documentRef.createElement("button"); action.type = "button"; action.className = "btn btn-secondary"; action.textContent = "Generate Draft"; action.disabled = !contact.selectedAt || selectionBlocked(contact); action.addEventListener("click", () => generate(contact).catch(handleError)); const actions = row.insertCell(); actions.className = "b2b-row-actions"; actions.appendChild(action);
        if (isSuperadmin) { const edit = documentRef.createElement("button"); edit.type = "button"; edit.className = "btn btn-secondary"; edit.textContent = "Edit"; edit.addEventListener("click", () => openContactEditor(contact)); actions.appendChild(edit); }
        if (isSuperadmin && !alreadySent(contact)) {
    ...
    async function markAsSent(contact) {
      if (!isSuperadmin || !contact || alreadySent(contact)) return;
    ...
    async function init() {
      ...
      el("b2bSaveContact")?.addEventListener("click", () => saveContact().catch(handleError)); el("b2bCancelContact")?.addEventListener("click", closeContactEditor); el("b2bEditContactClose")?.addEventListener("click", closeContactEditor); el("b2bEditContactModal")?.addEventListener("click", (event) => { if (event.target === el("b2bEditContactModal")) closeContactEditor(); });
